...
- **Local queue/cache ringan** dipakai HANYA sebagai buffer kondisi offline:
  - SEMUA operasi (transaksi baru, edit, hapus) yang gagal terkirim (gak ada internet) disimpen dulu di antrian lokal — TIDAK dibatasi cuma transaksi baru (sudah dipertimbangkan membatasi ke append-only demi kesederhanaan, tapi diputuskan tetap mendukung edit/hapus offline juga)
...
- **ID transaksi di-generate sebagai UUID v4 di sisi klien** (bukan auto-increment dari Supabase) — penting untuk idempotency: kalau proses retry sync terputus di tengah jalan dan dicoba ulang, transaksi TIDAK akan terduplikasi di database karena ID-nya sudah pasti sama
- **Resolusi konflik**: karena skenario penggunaan tipikal adalah 1 user di 1 device pada satu waktu (bukan multi-device simultan), strategi sederhana last-write-wins (berdasarkan timestamp lokal) sudah cukup — TIDAK perlu conflict resolution kompleks/CRDT
...
- **Liquid glass WAJIB terkondisi performa** — `BackdropFilter`/blur berat di GPU device entry-level (contoh: chipset kelas MediaTek Helio A/G series, termasuk device testing OPPO A3s). Sediakan fallback otomatis (deteksi frame drop) atau toggle manual "Mode Hemat Energi" yang mengganti efek blur dengan warna solid+opacity
- **Font Sora & Inter WAJIB di-bundle sebagai asset lokal**, BUKAN network fetch runtime dari `google_fonts` — app ini offline-first, tidak masuk akal kalau font butuh internet saat pertama dibuka
- **Package tambahan direkomendasikan**: `fl_chart` (grafik tren di Rekap), `skeletonizer` (loading skeleton otomatis dari widget asli, lebih maintainable daripada bikin skeleton manual), `pinput` (UI kotak OTP yang lebih rapi), `flutter_animate` (opsional, animasi deklaratif). Liquid glass SUDAH diimplementasikan native pakai `BackdropFilter` — tidak perlu package glassmorphism tambahan.
...
- **WAJIB ada fallback performa untuk device low-end** — `BackdropFilter` (blur) berat untuk GPU chipset entry-level (termasuk device testing KIRAIN sendiri, OPPO A3s). Deteksi frame rate drop atau sediakan toggle manual "Mode Hemat Energi" di Pengaturan yang mengganti blur dengan warna solid ber-opacity sebagai gantinya. Ini WAJIB diverifikasi langsung di OPPO A3s sebelum dianggap selesai — jangan cuma diasumsikan lancar dari testing di device lain.
...
### Splash Screen — Native vs Flutter (2 Layer Berbeda)
Ditemukan saat testing: ada 2 lapisan splash yang perlu ditangani terpisah.

1. **Native splash** (`flutter_native_splash`) — muncul sebelum Flutter engine jalan, level OS, **TIDAK bisa dianimasikan** (keterbatasan teknis Android/iOS, bukan pilihan desain). Cukup logo + background solid dari token warna (bukan default teal), durasi sangat singkat, tanpa crop/potong pada logo/wordmark.
2. **Flutter splash** (opsional, layar Flutter custom setelah native splash lewat, sebelum masuk Onboarding/Sign In) — DI SINI animasi bisa diterapkan:
   - Logo/wordmark "KIRAIN" fade-in + scale halus (~400ms)
   - Signature "alis kaget" digambar progresif (animasi path/stroke drawing, bukan muncul instan utuh) — merepresentasikan momen "reveal/kaget" sebagai elemen brand pertama yang dilihat user
   - Total durasi splash Flutter singkat (~1.5–2 detik), lalu otomatis lanjut ke layar berikutnya

### App Icon — FINAL (Konsep "Glass Depth")
Dipilih dari eksplorasi 2 ronde konsep. Menggunakan signature "alis kaget" yang sama dipakai di seluruh app, dengan treatment kaca yang konsisten dengan arah Liquid Glass yang sudah diadopsi (lihat bagian Liquid Glass).

**Spesifikasi:**
- **Background**: gradient linear diagonal (135°) dari mint terang (`#8FEAD1`) di kiri-atas → mint dasar (`#6BCBAF`) di tengah → mint lebih gelap (`#4FAE93`) di kanan-bawah — memberi kesan dimensi/kedalaman, bukan flat solid
- **Highlight kaca**: radial gradient putih transparan (opacity ~55% ke 0%) di area kiri-atas (sekitar 10-15% dari tepi), mensimulasikan pantulan cahaya di permukaan kaca
- **Mark**: dua lengkungan asimetris (alis kaget) — lengkungan kiri warna `#0D1412` (gelap/netral), lengkungan kanan warna `#FF8B5E` (coral) — dengan `drop-shadow` halus di bawah mark untuk kesan mengambang
- **Shape**: squircle sebagai mask utama (standar Android adaptive icon modern & iOS)
- **Ukuran mark**: sekitar 60-65% dari lebar icon, jangan sampai menyentuh tepi (pelajaran dari isu crop splash screen sebelumnya — selalu sisakan padding aman)

Implementasi: gradient dan highlight bisa dibuat sebagai layer background pada adaptive icon, mark sebagai layer foreground terpisah (standar Android adaptive icon: background + foreground layer).

### App Icon & Splash Screen (Legacy Note)
...
### Detail Implementasi — Local Write-Ahead Queue (klarifikasi teknis)
> Ditambahkan setelah masukan review Agustus 2026. PERLU DIVERIFIKASI ke Claude Code dulu: apakah local storage untuk queue offline sudah ada implementasinya (dan pakai package apa) — daftar dependencies yang dilaporkan sebelumnya tidak menyebutkan package database lokal (`sqflite`/`hive`/`isar`) sama sekali, jadi kemungkinan ini masih perlu ditambahkan dari awal, bukan migrasi.

- **ID transaksi**: WAJIB generate UUIDv4 di sisi client (device), BUKAN mengandalkan auto-increment ID dari Supabase. Ini membuat retry pengiriman transaksi (saat koneksi terputus di tengah proses) bersifat idempotent — tidak akan menduplikasi data meski request terkirim ulang.
- **Scope offline WAJIB dibatasi append-only untuk v1**: menambah transaksi baru BOLEH offline (masuk antrian lokal). Mengedit atau menghapus transaksi yang sudah ada WAJIB mensyaratkan koneksi internet aktif — ini scoping sengaja untuk menghindari kompleksitas conflict resolution yang tidak realistis dikerjakan solo developer di v1.
- Rekomendasi package (jika belum ada implementasi): `hive_ce` atau `isar` untuk local storage — lebih cepat dibanding `sqflite` untuk kebutuhan queue sederhana. Keputusan final menunggu konfirmasi status implementasi saat ini dari Claude Code.

### Notification Governance — Mencegah Notification Fatigue
> Ditambahkan setelah masukan review Agustus 2026. Dengan banyaknya sumber notifikasi yang sudah direncanakan (reminder harian, reminder gajian, recurring due, budget threshold per kategori, proactive check-in, peringatan prediktif "bakal boncos"), WAJIB ada mekanisme pembatasan supaya tidak terasa spam dan bertentangan dengan persona "asisten yang gak menghakimi".

- **Batas wajar**: maksimal ~1-2 notifikasi proaktif per hari (di luar notifikasi transaksional langsung seperti hasil sync), meskipun ada beberapa trigger yang secara teknis "layak" dikirim di hari yang sama — pilih yang paling relevan/mendesak, tunda sisanya
- **Prioritas saat bentrok**: peringatan kritis (Zona Kirain, budget prediktif akan jebol) > reminder aksi (gajian, recurring due) > check-in motivasional (proactive check-in "udah 3 hari nih")
- Tetap menggunakan notification channel yang sudah dipisah (Reminder Harian / Peringatan Budget / Update & Info) agar user tetap bisa mematikan kategori tertentu secara manual jika govern otomatis dirasa kurang

### Fitur Baru Tambahan — "Mindful Spending" (dari review Agustus 2026, DIANTRIKAN setelah Fase 3 & 4 fitur Financial Freedom sebelumnya)
> Disetujui masuk v1, TAPI prioritas di bawah penyelesaian layar Fase 3 dan 4 fitur Financial Freedom yang sudah diputuskan sebelumnya — jangan dikerjakan duluan hanya karena kedengaran menarik.

- **Opportunity Cost Converter** ("harga dalam jam kerja") — konversi harga transaksi kategori Keinginan ke estimasi jam kerja user (berdasarkan input gaji bulanan yang sudah ada). Contoh: "Ini setara 3 jam kerja kamu". Murni aritmatika lokal, tanpa API eksternal.
- **Cooling-off Timer + Shadow Savings Pot** (digabung jadi 1 fitur karena saling terkait) — untuk transaksi Keinginan di atas nominal tertentu (threshold bisa diset user), sediakan opsi "Tahan Dulu" yang menunda transaksi 24-48 jam alih-alih langsung dicatat. Kalau setelah timer selesai user membatalkan niat beli, nominal itu masuk ke agregat "Total yang Berhasil Diselamatkan" — efek motivasional tanpa infrastruktur tambahan.
- **Auto-Alokasi Hak saat Gajian** — begitu transaksi kategori Gaji/Pemasukan dicatat, tampilkan pop-up kalkulasi otomatis (persentase infak/zakat & tabungan bisa dikustomisasi user) dengan tombol one-tap buat langsung mengalokasikan ke pos masing-masing. Terhubung dengan reminder gajian yang sudah direncanakan sebelumnya.

## 12. Visi Jangka Panjang & "Financial Freedom"

> Ditambahkan Agustus 2026 — ambisi besar KIRAIN sebagai bintang penuntun roadmap, BUKAN daftar yang harus langsung selesai di v1. Setiap item di bawah tetap disaring lewat prinsip "moderate risk, low cost" sebelum masuk scope aktif manapun.

**Visi:** KIRAIN bukan sekadar pencatat transaksi — jadi "asisten" yang benar-benar mengawal penggunanya menuju *financial freedom*: mengingatkan, memperingatkan risiko sebelum terjadi, dan membangun kedisiplinan lewat kebiasaan kecil harian.

**4 pilar:**
1. **Kemudahan pencatatan** — sudah jadi fokus utama sejak awal (quick-add, capture effortless)
2. **Pengingat proaktif** — sudah ada rencana dasar (reminder gajian, recurring due); diperluas jadi check-in yang lebih "hidup"
3. **Peringatan risiko masa depan** — bukan cuma reaktif (Zona Kirain setelah kejadian), tapi prediktif (memperingatkan SEBELUM kejadian)
4. **Motivasi & kedisiplinan** — kerangka framing "menuju financial freedom", bukan cuma angka-angka kering

### Fitur Baru — Disetujui Masuk V1 (algoritma murni, gratis, risiko rendah)

- **Skor Kesehatan Finansial** — angka 0-100 dihitung dari data yang sudah ada: rasio wajib terpenuhi, streak konsistensi mencatat, savings rate ke Target Nabung, frekuensi defisit historis. Gamifikasi ringan, murni matematika, tanpa biaya tambahan.
- **Peringatan prediktif "bakal boncos"** — proyeksi linear sederhana dari rate pengeluaran vs sisa hari di siklus berjalan (contoh: *"Di rate sekarang, budget wajib bakal habis 5 hari sebelum siklus berakhir"*). Murni matematika (rate × sisa hari), tanpa AI.
- **Proactive check-in dari "asisten"** — notifikasi kontekstual berbasis pola pemakaian (contoh: *"Udah 3 hari nih belum kecek KIRAIN, gimana kabar duitmu?"*), memperkuat kesan diawasi/diingatkan sesuai persona asisten. Murni logic notifikasi lokal.
- **Framing "Financial Freedom" sebagai narasi** — bukan fitur teknis baru, tapi cara membingkai fitur yang sudah ada (Dana Darurat, Target Nabung, streak, Skor Kesehatan Finansial) sebagai *"progress menuju kebebasan finansial"* di Rekap/laporan berkala. Perubahan copy/framing, bukan logic baru.

### Fitur Baru Tambahan — Hasil Konsultasi Gemini (disetujui masuk V1)

- **Opportunity Cost Converter ("harga dalam jam kerja")** — konversi nominal transaksi kategori Keinginan ke jam kerja setara, berdasarkan pemasukan/gaji yang sudah diinput user (contoh: *"Ini setara 3 jam kerja kamu"*). Murni aritmatika lokal (gaji bulanan ÷ estimasi jam kerja per bulan), tanpa API eksternal. Selaras kuat dengan filosofi inti "mikir dulu sebelum belanja".
- **Auto-Alokasi Hak saat gajian** — begitu transaksi Pemasukan/Gaji dicatat, tampilkan pop-up kalkulasi instan alokasi (misal 2.5% zakat/infak, persentase tabungan yang bisa disesuaikan user) dengan tombol one-tap buat langsung alokasikan ke pos masing-masing (kategori Infak/Zakat, Target Nabung). Extension natural dari reminder gajian yang sudah direncanakan.
- **Impulse Cooling-off Timer ("Tahan Dulu")** — untuk transaksi kategori Keinginan di atas nominal tertentu (threshold bisa disesuaikan user), sediakan opsi "Tahan Dulu" yang memasukkan item ke daftar tunda selama 24–48 jam (dengan reminder lokal saat waktu habis) sebelum benar-benar dicatat sebagai transaksi. **Catatan effort**: ini BUKAN fitur ringan — butuh state/tabel baru untuk daftar tunda dan notifikasi terjadwal — tapi disetujui masuk V1 karena selaras kuat dengan "decision layer" (diferensiasi utama KIRAIN).
- **Shadow Savings Pot ("Total yang Berhasil Diselamatkan")** — setiap kali user membatalkan rencana belanja dari Impulse Cooling-off Timer (tidak jadi checkout setelah masa tunda), catat nominalnya ke agregat "Total Keuangan Diselamatkan" yang ditampilkan sebagai motivasi. Bergantung pada Impulse Cooling-off Timer sebagai sumber event pembatalan. Efek psikologis tinggi, tanpa infrastruktur backend tambahan (hanya field agregat).

### Catatan Blind Spot (dari konsultasi Gemini, untuk terus diperhatikan)

- **Notification fatigue** — kombinasi peringatan prediktif + proactive check-in + reminder lainnya berisiko terasa menghakimi/melelahkan kalau terlalu sering. Pastikan nada tetap sesuai persona "santai yang serius pas dibutuhkan" (bagian 2 & 6) — pendekatan mikro-rekomendasi, bukan cuma peringatan bahaya bertubi-tubi.
- **Kontras warna mint/coral di atas permukaan liquid glass translucent** — ini kasus khusus di luar perbaikan kontras yang sudah dilakukan (lihat "Koreksi & Gap"), karena latar belakang translucent/blur membuat kontras bisa bervariasi tergantung konten di baliknya. Pastikan teks di atas permukaan glass tetap diuji rasio kontrasnya, terutama untuk kondisi pemakaian di luar ruangan/cahaya terang.



### Riset AI — Referensi untuk V1.1/V2 (BUKAN scope V1 sekarang)
> Keputusan: v1 fokus algoritma matematis murni (di atas), TANPA AI. Riset ini disimpan sebagai referensi kalau nanti mau dieksplorasi.

Potensi use-case: parsing transaksi natural language (misal ketik/ucap "50rb makan siang" auto-terisi jadi transaksi), kategorisasi otomatis pintar. Referensi nyata: app finance "Jibify" sudah menerapkan kombinasi ini di Play Store.

**Opsi A — On-device (Gemini Nano via ML Kit GenAI Prompt API)**
- Gratis 100%, jalan offline, data tidak pernah keluar device — sejalan dengan prinsip privacy-first KIRAIN
- **Keterbatasan besar**: hanya jalan di device flagship (Pixel 8+, Samsung Galaxy S24+, chipset Snapdragon 8 Gen 3+). Device testing KIRAIN saat ini (OPPO A3s, Galaxy Z Flip5) kemungkinan TIDAK didukung — baru bisa jadi fitur bonus untuk sebagian device, harus ada fallback graceful ke input manual

**Opsi B — Cloud API (Gemini Flash-Lite, tier berbayar)**
- Sangat murah untuk tugas ringan: ~$0,10/1M token input, ~$0,40/1M token output. Estimasi kasar: 1000 user × 10x parsing/hari ≈ $9/bulan
- **WAJIB pakai tier berbayar, BUKAN tier gratis** — tier gratis Gemini menyatakan data bisa dipakai untuk melatih ulang produk Google, ini bertentangan dengan janji privacy KIRAIN. Kalau nanti dipakai, privacy policy harus diupdate untuk transparan soal ini.
- Bekerja di semua device (tidak bergantung hardware flagship)

**Rekomendasi kalau nanti dieksekusi:** kombinasi — cloud Flash-Lite berbayar sebagai jalur utama (konsisten di semua device, biaya kecil), on-device Gemini Nano sebagai bonus opsional untuk device yang mendukung (fallback otomatis).
